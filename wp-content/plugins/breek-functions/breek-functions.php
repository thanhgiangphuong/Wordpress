...
defined( 'ABSPATH' ) or die( 'No script kiddies please!' ); // No direct access allowed
 ...
 if ( !class_exists('epcl_breek_theme_functions') ) {
    ...
    if( !defined('EPCL_THEMESLUG') ) define('EPCL_THEMESLUG', 'breek');

    class epcl_breek_theme_functions {

        public function __construct() {

            /* Check if Breek Theme is activated */

            if( get_template() != 'breek' ){
                return;
            }
            ...
            /* Canonical URL's custom homepage */

            // add_action( 'wp_head', array( $this, 'epcl_add_canonical_url'), 1 );
            add_filter( 'get_canonical_url', array($this, 'epcl_change_canonical'), 1 ); 
            add_filter( 'wpseo_canonical', array($this, 'epcl_change_canonical'), 1 ); 
        ...
        public function insert_fb_in_head() {
            ...
            if( isset($epcl_theme['enable_open_graph']) && $epcl_theme['enable_open_graph'] !== '0' ){
                ...
                // Post or pages
                if( is_singular() ){
                    if(!has_post_thumbnail( $post->ID ) && isset($epcl_theme['logo_image']) && isset($epcl_theme['logo_image']['url']) && $epcl_theme['logo_image']['url'] != '' ) { // No image
                        $image = $epcl_theme['logo_image']['url'];
                    }else{
        ...
        public function epcl_add_canonical_url(){
            global $wp;
            ...
            if( is_page_template('page-templates/home.php') ){
                if( $prev_url && $prev_url !== esc_url( $current_url ) ){
                    echo '<link rel="prev" href="'.esc_url($prev_url).'" />';
                }
                if( $next_url && esc_url($next_url) !== esc_url( $current_url ) ){
        ...
        function epcl_change_canonical($url) {
            global $wp;
